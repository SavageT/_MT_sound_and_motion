...
% display.dist = 57; %%50; %laptop % 57 display++
% display.width = 70; %28.5; %laptop % 70 display++
...
% display.height=33.528;% in cm
% display.pxPerDeg=46.3;
...
for block = 1:size(p.order,1)
    ...
    while GetSecs < startTime + p.blockDur*(block-1)+2.75
        ...
        movingDots_orig(display, dots, 0.5)% 500ms dots
    end
    ...
    while GetSecs < startTime + p.blockDur*(block-1) + 5.25
        ...
        if keyIsDown
            ...
            if any(exp.resp==27); br=1;break;end
